...
var MemStorage = class {
  ...
  constructor() {
    ...
    this.bookingRequests = /* @__PURE__ */ new Map();
  }
  async getUser(id) {
  ...
  async createBookingRequest(insertBookingRequest) {
    const id = randomUUID();
...
async function registerRoutes(app2) {
  app2.post("/api/booking-requests", async (req, res) => {
    try {
  ...
  app2.get("/api/booking-requests", async (req, res) => {
    try {
      const bookingRequests2 = await storage.getBookingRequests();
      res.json(bookingRequests2);
    } catch (error) {
      res.status(500).json({
        message: "Internal server error"
      });
...
var vite_config_default = defineConfig({
  base: "./",
  // 👈 this makes asset paths relative (./assets/...)
  plugins: [
    react(),
    runtimeErrorOverlay(),
